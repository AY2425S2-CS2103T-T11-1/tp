...
public class EditCommandParserTest {
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + INVALID_YEAR_DESC, Year.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + INVALID_MAJOR_DESC, Major.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + INVALID_HOUSING_DESC, Housing.MESSAGE_CONSTRAINTS);
        ...
        assertParseFailure(parser, "1" + INVALID_NAME_DESC + INVALID_EMAIL_DESC + VALID_HOUSING_AMY + VALID_PHONE_AMY,
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + TAG_DESC_HUSBAND
                + EMAIL_DESC_AMY + YEAR_DESC_AMY + MAJOR_DESC_AMY + HOUSING_DESC_AMY
                + NAME_DESC_AMY + TAG_DESC_FRIEND;

        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withName(VALID_NAME_AMY)
                .withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_AMY)
                .withYear(VALID_YEAR_AMY).withMajor(VALID_MAJOR_AMY).withHousing(VALID_HOUSING_AMY)
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        // housing
        userInput = targetIndex.getOneBased() + HOUSING_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder().withHousing(VALID_HOUSING_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    ...
    public void parse_multipleRepeatedFields_failure() {
        ...
        // multiple valid fields repeated (including year, major, housing)
        userInput = targetIndex.getOneBased()
                + PHONE_DESC_AMY + HOUSING_DESC_AMY + EMAIL_DESC_AMY + YEAR_DESC_AMY + MAJOR_DESC_AMY
                + TAG_DESC_FRIEND
                + PHONE_DESC_AMY + HOUSING_DESC_AMY + EMAIL_DESC_AMY + YEAR_DESC_AMY + MAJOR_DESC_AMY
                + TAG_DESC_FRIEND
                + PHONE_DESC_BOB + HOUSING_DESC_BOB + EMAIL_DESC_BOB + YEAR_DESC_AMY + MAJOR_DESC_AMY
                + TAG_DESC_HUSBAND;

        assertParseFailure(parser, userInput,
                Messages.getErrorMessageForDuplicatePrefixes(
                        PREFIX_PHONE, PREFIX_EMAIL, PREFIX_HOUSING, PREFIX_YEAR, PREFIX_MAJOR));

        // multiple invalid values (including year, major, housing)
        userInput = targetIndex.getOneBased()
                + INVALID_PHONE_DESC + INVALID_HOUSING_DESC + INVALID_EMAIL_DESC + INVALID_YEAR_DESC
                + INVALID_MAJOR_DESC + INVALID_PHONE_DESC + INVALID_HOUSING_DESC + INVALID_EMAIL_DESC
                + INVALID_YEAR_DESC + INVALID_MAJOR_DESC;
