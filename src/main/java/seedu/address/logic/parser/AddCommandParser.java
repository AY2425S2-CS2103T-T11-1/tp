    ...
    public AddCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_NAME, PREFIX_PHONE,
                        PREFIX_EMAIL, PREFIX_YEAR, PREFIX_MAJOR, PREFIX_HOUSING, PREFIX_LINK, PREFIX_TAG);

        // Checks whether mandatory fields are present
        if (!arePrefixesPresent(argMultimap, PREFIX_NAME, PREFIX_HOUSING, PREFIX_PHONE,
                PREFIX_EMAIL, PREFIX_YEAR, PREFIX_MAJOR)
                || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddCommand.MESSAGE_USAGE));
        }

        argMultimap.verifyNoDuplicatePrefixesFor(PREFIX_NAME, PREFIX_PHONE, PREFIX_EMAIL, PREFIX_HOUSING,
                PREFIX_YEAR, PREFIX_MAJOR, PREFIX_LINK);
